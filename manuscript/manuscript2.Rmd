...
\subsection{Data}
 The data used in this study is the National Health and Nutrition Examination Survey data (NHANES)[@NHANES]. The survey examines a nationally representative sample of the U.S. population focusing on a variety of health and nutrition measurements releasing two year cycle.  In this study, we accumulated 6 cycles of NHANES data (2007–2018). We used two data files: One contains demographic variables, such as age, sex, race, income, etc. and the other contains data that are related to body measurements, such as BMI, head circumference, etc. These files are merged by using the respondent sequence number (SEQN) resulting in a around 12,000 records. We selected a population age between 20 and 80.
...
From Figures \@ref(fig:resu1), we can see the effects on conditional mean of BMI level may not reflect the size and nature of these effects on lower or upper quantiles. For example, the conditional mean effect of gender on BMI level is about -1 BMI unite, that is, on average a male BMI is less than  female BMI by one BMI unit. However, at the lower quantiles males have higher BMI by 1 BMI unite. Then, the differences start to diminish up to 0 at 40th quantile. At higher quantiles, a male BMI is less than a female BMI. For example, at 60th quantile a male BMI is less than a female BMI by 1 unite, and it continue to increase up to 3 BMI unites at 90th quantile.     


From the OLS it is obvious cholesterol drug users have on average higher BMI levels if compared to non-cholesterol drug users which are around 1.75. The disparity is almost regular through different BMI quantiles. For example, in the lower quantiles of the distribution the cholesterol drug users have higher BMI value by about 1.5 units, but around at 50 th percentile of the conditional distribution the difference is 1.8 BMI units. Overall,  cholesterol drug use seems to be associated with rather large effects on BMI levels somewhat between  1 to 2 BMI units.  
...
Figure \@ref(fig:resu1000) illustrate the differences in modeling Age in two different ways (Model I; spline basis expansion) and (Model III; quadratic factor ).  
 Model I shows, at the lower quantile, the marginal age effects on BMI increase from age 20 to around age 75, and it starts to decrease after that. While model III shows there is a positive relationship in general. In the 50th quantile, model I shows age effect on BMI almost consistent result with the 0.1th quntile, but model III shows age effects approximately has flat effects. At a $(\tau=0.75)$ quantile of the conditional BMI distribution, model I shows a strong positive relationship between age and BMI for the age period from 20 to 33, and after that a negative  relationship, while model III shows a mild negative relationship in general. At a higher quantile $(\tau=0.9)$ of the conditional BMI distribution, we have a similar behavior as in 0.75th quantile except the relationship is stronger.The BMI trend in this modeling is close to the trend shown in [@chen2005] by using a complicated polynomial and log transformation for the response. Our conclusion from Figure \@ref(fig:resu1000) is that the marginal age effects on BMI is different on different quantiles. Moreover, modeling age as quadratic term or as spline basis expansion produce different effect with respect to differnt  quantile of the conditional BMI distribution. 
...
Next, we will investigate marginal effect of TC on four different quntiles of the conditional distribution of the BMI. 
